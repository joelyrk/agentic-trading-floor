...
def test_diagnostics_are_redacted_and_bounded() -> None:
    diagnostic = safe_error(
        "authorization=Bearer-secret sk-" + "abcdefghijklmnopqrstuvwxyz " + "x" * 600
    )
...
def test_failed_mcp_startup_attributes_redacted_subprocess_diagnostic(
    ...
    server = ObservedMCPServerStdio(
        {
            ...
            "args": [
                "-c",
                "import sys; print('api_key=never-store-this', file=sys.stderr)",
            ],
        },
