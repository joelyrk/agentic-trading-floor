...
`MARKET_DATA_FRESHNESS_SECONDS` defaults to 300 seconds for simulated observations and four days for EOD observations so a Friday close remains usable over an ordinary weekend. `MARKET_DATA_CACHE_SECONDS` defaults to one hour for EOD observations and zero for simulated data; cache hits retain the original source and retrieval timestamp, and a new provider request is made after expiry. `MARKET_DATA_TIMEOUT_SECONDS` defaults to 10 seconds. Massive timeouts, HTTP 429 responses, and HTTP 5xx responses are retried twice by default with exponential backoff beginning at `MARKET_DATA_RETRY_BACKOFF_SECONDS=0.5`; authentication, entitlement, and other failures are not retried. `MARKET_DATA_MAX_RETRIES` controls the bounded retry count. This phase supports only `simulated` and `end_of_day`; requesting `delayed` or `real_time`, or requesting EOD without a key, fails startup with an actionable configuration error.
