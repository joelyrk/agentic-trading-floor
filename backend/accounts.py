...
class Account(BaseModel):
    ...
    def buy_shares(self, symbol: str, quantity: int, rationale: str) -> str:
        ...
        observation_id = write_market_observation(
            self.name, "order", order_id, observation
        )
        # Record transaction
    ...
    def sell_shares(self, symbol: str, quantity: int, rationale: str) -> str:
        ...
        if self.holdings.get(normalized_symbol, 0) < quantity:
            raise ValueError(f"Cannot sell {quantity} shares of {normalized_symbol}. Not enough shares held.")
    ...
    def report(self) -> str:
        ...
        self.portfolio_value_time_series.append((datetime.now(timezone.utc).isoformat(), portfolio_value))
        self.save()
