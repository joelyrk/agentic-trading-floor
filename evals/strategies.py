...
class SingleAgent:
    ...
    def decide(self, fixture, seed):
        ...
        ranked = max(fixture.trailing_returns, key=lambda symbol: (fixture.trailing_returns[symbol], symbol))
...
class MultiAgent:
    ...
    def decide(self, fixture, seed):
        sentiment = sum((source.sentiment for source in fixture.sources), Decimal("0"))
        ranked = sorted(fixture.trailing_returns, key=lambda symbol: (fixture.trailing_returns[symbol], symbol), reverse=True)
...
def default_strategies() -> list[Strategy]:
    return [BuyAndHold(), EqualWeight(), NoTrade(), RandomValidTrades(), Momentum(), SingleAgent(), MultiAgent()]
